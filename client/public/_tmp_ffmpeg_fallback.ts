    ...
    try {
      ...
      const baseFromVite =
        ...
        typeof import.meta.env.BASE_URL === "string"
          ...
          : "/";
      const normalizedBase = normalizeBaseUrl(baseFromVite);
      const localCandidates = useMultiThread
        ? (["core-mt", "core"] as const)
        : (["core"] as const);

      const loadFromLocalCore = async (variant: "core-mt" | "core"): Promise<FFmpegInstance> => {
        const ffmpeg = new FFmpeg() as unknown as FFmpegInstance;
        const baseUrl = joinUrl(normalizedBase, `ffmpeg/${variant}`);
        try {
          if (variant === "core-mt") {
            await ffmpeg.load({
          ...
          try {
            ...
            // Ignore cleanup errors for failed init attempts.
          }
          throw error;
        }
      };

      let loadedFfmpeg: FFmpegInstance | null = null;
      const errors: string[] = [];
      for (const variant of localCandidates) {
        try {
          loadedFfmpeg = await loadFromLocalCore(variant);
          break;
        } catch (loadError) {
      ...
      if (!loadedFfmpeg) {
        throw new Error(
          `[FFmpeg] Local core load failed. Tried ${localCandidates.join(" -> ")}. ${errors.join(" | ")}`,
        );
      }

      this.ffmpeg = loadedFfmpeg;
      this.loaded = true;
    } catch (error) {
      this.loading = null;

...
export type PptxPdfMode = "narrative_summary" | "faithful_accessible";
...
type JobStatus = {
  ...
  error?: string;
  documentName?: string;
  outputMode?: PptxPdfMode;
  technicalCompliance?: "pending" | "pass" | "fail";
  riskFlags?: string[];
  degraded?: boolean;
  degradedReasons?: string[];
  qaReportAvailable?: boolean;
  queue?: {
...
type HealthState<TPayload extends { error?: string }> = {
...
const PHASE_LABELS: Record<string, string> = {
  ...
  vision: "Visuelle Struktur wird analysiert...",
  gpu_queue: "Wartet auf GPU...",
  vlm: "KI-Analyse (Alt-Texte & Diagramme)...",
  summary: "Lesefassung wird aufgebaut...",
  building: "Struktur wird generiert...",
  pdfua: "PDF/UA wird geprüft...",
...
const RISK_FLAG_LABELS: Record<string, string> = {
  hidden_text_leak: "Versteckter Text wurde erkannt und unterdrückt",
  pseudo_table: "Tabellenähnliche Struktur wurde bewusst nicht als Tabelle gerendert",
  duplicate_content: "Doppelte Inhalte wurden erkannt",
  degraded_gpu: "GPU-Schritte wurden reduziert",
  off_slide_text: "Text außerhalb der sichtbaren Folie wurde gefiltert",
  master_artifact: "Master- oder Layout-Artefakte wurden gefiltert",
};

function renderRiskFlag(flag: string) {
  return RISK_FLAG_LABELS[flag] || flag.replace(/_/g, " ");
}

export function PptxPdfModePage(config: ModeConfig) {
  const { user } = useAuth();
  ...
  const healthStatus = String(summaryPdfHealth.payload?.status || "").trim().toLowerCase();
  const serviceError =
  ...
  useEffect(() => {
    ...
    const fetchHealth = async (): Promise<HealthState<SummaryPdfHealthEnvelope>> => {
      try {
        ...
        const available = normalized === "healthy" || normalized === "ok" || normalized === "degraded";
        return {
          available,
          payload,
          error: available ? undefined : payload.error || "Service meldet keinen verfügbaren Status",
        };
      } catch (err: any) {
        return {
          available: false,
  ...
  useEffect(() => {
    ...
    const cancelJob = () => {
      const url = `${config.apiBase}/cancel/${jobId}`;
  ...
  }, [config.apiBase, isProcessing, jobId]);
  ...
  const pollJobStatus = useCallback(
    async (id: string) => {
      try {
        const response = await fetchWithAuthRetry(`${config.apiBase}/status/${id}`);
        ...
        if (jobStatus.status === "completed") {
          ...
          toast({
            ...
            description:
              jobStatus.degraded
                ? "Die Ausgabe ist bereit. Einige GPU-Schritte wurden reduziert und im Qualitätsstatus markiert."
                : "Das barrierefreie PDF/UA ist bereit zum Download.",
          });
          return;
        }
    ...
    [config.apiBase, config.completionTitle, fetchWithAuthRetry, toast],
  );
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchWithAuthRetry(`${config.apiBase}/convert`, {
        method: "POST",
        body: formData,
      });
  ...
  }, [config.apiBase, fetchWithAuthRetry, file, notifyByEmail, pollJobStatus, serviceAvailable, serviceError, toast, user?.email]);

  const handleDownload = useCallback(async () => {
    if (!jobId) return;
    try {
      const response = await fetchWithAuthRetry(`${config.apiBase}/download/${jobId}`);
      if (!response.ok) {
        if (response.status === 401) {
          throw new Error("Sitzung abgelaufen. Bitte neu einloggen.");
        }
  ...
  }, [config.apiBase, fetchWithAuthRetry, file?.name, jobId, toast]);
  ...
  const qualityMessages = [
    ...
    ...((status?.riskFlags || []).map(renderRiskFlag)),
  ];

  return (
    <PageLayout>
      <SEO title={config.seoTitle} description={config.seoDescription} canonical={config.canonical} />
      ...
      <main id="main-content" className="max-w-4xl mx-auto px-6 py-8" tabIndex={-1}>
        ...
        <div className="text-center mb-12">
          <div className={`w-20 h-20 ${config.accentClass} rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-lg`}>
            <FileOutput className="w-10 h-10 text-white" />
          </div>
          <h1 className="text-4xl font-semibold text-slate-900 mb-4">{config.heroTitle}</h1>
          <p className="text-xl text-slate-600 max-w-2xl mx-auto">{config.heroDescription}</p>
        </div>
        ...
        {serviceAvailable !== false && healthStatus === "degraded" && (
          <div className="mb-8 p-4 bg-blue-50 border border-blue-200 rounded-xl flex items-start gap-3">
            <ShieldCheck className="w-5 h-5 text-blue-600 mt-0.5 shrink-0" />
            <div>
              <p className="font-medium text-blue-900">Dienst verfügbar, aber degradiert</p>
              <p className="text-sm text-blue-800">
                Die Konvertierung ist verfügbar. Qualitätswarnungen oder reduzierte GPU-Schritte werden im Jobstatus sichtbar gemacht.
              </p>
            </div>
          </div>
        )}
          ...
          {!isProcessing && !outputReady && (
            <div className="p-8">
              <div className="mb-8 p-4 bg-slate-50 border border-slate-200 rounded-xl">
                ...
                <p className="text-sm text-slate-600">{config.introDescription}</p>
                <p className="text-xs text-slate-500 mt-2">{config.detailNote}</p>
              </div>
              ...
              {file && (
                <div className="mt-6 flex flex-col items-center gap-4">
                  ...
                  <Button
                    ...
                    className={`h-14 px-8 text-lg ${config.accentClass} hover:opacity-95 rounded-xl shadow-lg`}
                  >
            ...
            <div className="p-8" role="status" aria-live="polite" aria-atomic="true">
              <div className="flex flex-col items-center">
                ...
                {qualityMessages.length > 0 && (
                  <div className="mt-6 w-full max-w-2xl rounded-xl border border-blue-200 bg-blue-50 p-4 text-left">
                    <p className="text-sm font-medium text-blue-900 mb-2">Qualitätsstatus</p>
            ...
            <div className="p-8">
              <div className="flex flex-col items-center text-center">
                ...
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium bg-green-100 text-green-800 mb-4">
                  {status?.technicalCompliance === "pass" ? "Technisch geprüft" : "Barrierefrei erstellt"}
                </div>

                <h2 className="text-2xl font-semibold text-slate-900 mb-2">{config.completionTitle}</h2>
                ...
                {(status?.technicalCompliance || qualityMessages.length > 0) && (
                  <div className="mb-6 w-full max-w-2xl rounded-xl border border-slate-200 bg-slate-50 p-4 text-left">
                    <p className="text-sm font-medium text-slate-900 mb-2">Qualitäts- und Compliance-Status</p>
                    <p className="text-sm text-slate-700">
                      Technische PDF/UA-Prüfung:{" "}
                      <strong>{status?.technicalCompliance === "pass" ? "bestanden" : status?.technicalCompliance === "fail" ? "nicht bestanden" : "ausstehend"}</strong>
                    </p>
                    {qualityMessages.length > 0 && (
                ...
                <div className="flex flex-wrap gap-4 justify-center">
                  <Button
                    ...
                    className={`h-12 px-6 ${config.accentClass} hover:opacity-95 rounded-xl`}
                  >
